...
namespace cloudsoft
{
	...
	public partial class SerializerStream
	{
		...
		public int WriteDictionary(  IDictionary value)
		{
			int dataSize = 0;
			...
			if( dic == null || dic.Count == 0 ){
				...
				foreach( var key in dic.Keys )
				{
					...
					if( _value != null )
					{
						type = dic[ key].GetType();

						if( hasSubtype )
						{
							SubtypeSerializer ss = ser2.GetSubtype( ref type );
							WriteSystem_UInt32( ss.Id );
							dataSize += SerializerSystem.Write(this,dic[ key], ss.Ser);
						}
						else
						{
							dataSize += SerializerSystem.Write(this,dic[ key], ser2);
						}
					}
					i++;
				}
